...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var b = false
        if (!pf.find("ta").equals("")) b = true
        galleryAdapter = GalleryAdapter(
                arrayListOf(
                        GalleryItem("ta", "Ane Naru Mono 7",
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.action_bookmarks -> {
                Toast.makeText(applicationContext, "Bookmarks", Toast.LENGTH_SHORT).show()
                var intent = Intent(applicationContext, BookmarkActivity::class.java)
                startActivity(intent)
                return true
            }
            R.id.action_downloads -> {
                Toast.makeText(applicationContext, "Downloads", Toast.LENGTH_SHORT).show()
                return true
            }
